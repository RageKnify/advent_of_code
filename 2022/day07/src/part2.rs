...
impl Dir {
    ...
    fn find_smallest(&self, target: u64) -> Option<u64> {
        if let Some(best_child) = self
            .dirs
            .values()
            .flat_map(|c| c.find_smallest(target))
            .min()
        {
    ...
    fn result(&mut self) -> u64 {
        const MAX_SIZE: u64 = 70_000_000;
        const REQUIRED_SIZE: u64 = 30_000_000;
        self.calculate_sizes();
        let free = MAX_SIZE - self.size;
        let needed = REQUIRED_SIZE - free;
        self.find_smallest(needed).unwrap()
    }
}
